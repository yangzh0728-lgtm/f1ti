(function (root, factory) {
  ...
  function calculateF1Result(answerIds) {
    ...
    const maxPossible = f1Questions.length * 5;
    const matches = f1Drivers
      .map((driver) => ({
        driver,
        score: scores[driver.id],
        percent: Math.max(1, Math.round((scores[driver.id] / maxPossible) * 100))
      }))
    ...
    return {
      ...
      score: topScore,
      percent: Math.max(1, Math.round((topScore / maxPossible) * 100)),
      matches
    };
  }
